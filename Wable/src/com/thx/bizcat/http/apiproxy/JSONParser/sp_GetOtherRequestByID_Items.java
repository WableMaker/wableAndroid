...
public class sp_GetOtherRequestByID_Items {
	public List<sp_GetOtherRequestByID_Result> requests = new ArrayList<sp_GetOtherRequestByID_Result>();
	public boolean bsuccess = false;
	public ResultCode resultCode;

	public sp_GetOtherRequestByID_Items(JSONObject obj) {
		...
		try {
			...
			if(bsuccess) {
				...
				for(int i=0,m=arr.length(); i<m; i++) 
					requests.add(new sp_GetOtherRequestByID_Result(arr.getJSONObject(i)));
			}
