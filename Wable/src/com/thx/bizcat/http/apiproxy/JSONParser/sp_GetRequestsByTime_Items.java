...
public class sp_GetRequestsByTime_Items {
	...
	public List<sp_GetRequestsByTime_Result> requests = new ArrayList<sp_GetRequestsByTime_Result>();
	public boolean bsuccess = false;
	public ResultCode resultCode;
	public sp_GetRequestsByTime_Items(JSONObject obj) {
		...
		try {
			...
			if(bsuccess) {
				...
				for(int i=0,m=arr.length(); i<m; i++) 
					requests.add(new sp_GetRequestsByTime_Result(arr.getJSONObject(i)));
			}
